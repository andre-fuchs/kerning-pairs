...
minimum = max(useCounts) * 0.2  # Clamp at 20 % ... removes unusual pairs in a pragmatic way 
print('Clamp Minimum', minimum)
realWorldKerningPairs = {k: v for k, v in realWorldKerningPairs.items() if v[0] > minimum}
...
difference = [x for x in tqdm(potentialKerningPairs) if x not in set(realWorldKerningPairs)]
...
for LANGUAGE in LANGUAGES:
    with open('count/by_language/' + LANGUAGE + '/list.json', 'r') as input:
        ...
        print(LANGUAGE, 'Letter Pairs', len(letterPairs), 'Total Count', "{:,}".format(totalValue))
        with open('count/by_language/' + LANGUAGE + '/common_kerning_pairs.json', 'w', encoding='utf8') as output:
            commonKerningPairs = {key: letterPairs[key] for key in potentialKerningPairs if key in letterPairs}  # Filter potential kerning pairs
            commonKerningPairs = {k: v/totalValue * 3000 * 100 for k, v in commonKerningPairs.items()}  # Normalize 1 in 100 pages a 3000 characters
            commonKerningPairs = sorted(commonKerningPairs.items(), key=operator.itemgetter(1), reverse=True)  # Sort by count
            # print(commonKerningPairs)
            output.write(json.dumps(commonKerningPairs, indent=4, ensure_ascii=True))  # Output
...
globalKerningPairs = {}
for LANGUAGE in LANGUAGES:
    with open('count/by_language/' + LANGUAGE + '/common_kerning_pairs.json', 'r') as input_a:
        kerningPairs = dict(json.load(input_a))
        # Stores the highest existing value of the given languages
        globalKerningPairs = { key: kerningPairs.get(key, 0) \
            if kerningPairs.get(key, 0) > globalKerningPairs.get(key, 0) else globalKerningPairs.get(key, 0) \
...
relevantGlobalKerningPairs = {k: v for k, v in globalKerningPairs.items() if v >= minimum}


# Sorting
globalKerningPairs = sorted(globalKerningPairs.items(), key=operator.itemgetter(1), reverse=True)
relevantGlobalKerningPairs = sorted(relevantGlobalKerningPairs.items(), key=operator.itemgetter(1), reverse=True)


# Output
with open('count/total/common_kerning_pairs.json', 'w') as output_a, \
     open('count/total/relevant_kerning_pairs.json', 'w') as output_c:
