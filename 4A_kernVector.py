...
# Praxis
with open("count/fonts/googleFontsKernDumpList.json", "r") as input_a, open(
    "count/total/relevant_kerning_pairs.json", "r"
) as input_b, open(
    "count/fonts/googleFontsKernVectorDictionary.json", "w"
) as output_a, open(
    "count/fonts/googleFontsKernVectorList.json", "w"
) as output_b:
    ...
    useCountsClamped = [
        value[0] for value in kernDump.values() if value[0] > minimum
    ]  ## clamp at > 2 %
