...
for SUFFIX, SET in CHARACTER_SETS.items():
    with open("count/total/relevant_kerning_pairs.json", "r") as input_json, open(
        "count/total/relevant_kerning_pairs_" + SUFFIX + ".json", "w"
    ) as output_json, open(
        "count/total/relevant_kerning_pairs_" + SUFFIX + ".txt", "w"
    ) as output_txt:
        relevantKerningPairs = json.load(input_json)
        filtered = [
            k
            for k in relevantKerningPairs
            if any(character in k[0] for character in SET)
        ]
...
# Inverse
with open("count/total/relevant_kerning_pairs.json", "r") as input_json, open(
    "count/total/relevant_kerning_pairs_buchstaben_only.json", "w"
) as output_json, open(
    "count/total/relevant_kerning_pairs_buchstaben_only.txt", "w"
) as output_txt:
    relevantKerningPairs = json.load(input_json)
    filtered = [
        k
        for k in relevantKerningPairs
        if not any(character in k[0] for character in CHARACTER_SETS["non-letters"])
    ]
    kerningTXT = ["nonu" + k + "nuou\n" for (k, v) in filtered]
    # pprint(filtered)
